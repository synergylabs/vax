...
def create_vpu_pipeline():
    ...
    if pose_model_config["color_order"] == "BGR":
        camRgb.setColorOrder(dai.ColorCameraProperties.ColorOrder.BGR)
    elif pose_model_config["color_order"] == "RGB":
        camRgb.setColorOrder(dai.ColorCameraProperties.ColorOrder.BGR)
    else:
        raise ValueError("Invalid color order: {}".format(
            pose_model_config["color_order"]))
    camRgb.setPreviewSize(*pose_model_config["input_size"])
    ...
    blob = (pose_model_config["blob"] + "_sh6.blob")
            ...
            while True:
                ...
                if latestPacket["rgb"] is not None:
                    ...
                    if curr_keypoints is not None:
                        keypointFrame = create_keypoint_frame(curr_keypoints, pose_model_config['input_size'][0],
                                                              pose_model_config['input_size'][1], frameRgb.shape[1],
                                                              frameRgb.shape[0])
                        curr_time = datetime.datetime.now().strftime("%H%M%S")
                        cv2.imwrite(f"cache/pose_eval/raw/{curr_time}.png",frameRgb)
                        frameRgb = cv2.add(frameRgb,keypointFrame)
                        rgb_recorder_queue.put((frame_time, frameRgb))
                        cv2.imwrite(f"cache/pose_eval/oakpose/{curr_time}.png",frameRgb)
                    if VISUALIZE:
