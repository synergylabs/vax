...
class RGBRecorderThread(threading.Thread):
    def __init__(self, rgb_queue, logger, write_folder, Config, video_prefix='rgb'):
        ...
        self.max_frames_per_video = self.config.max_duration_per_video * self.config.rgb_fps * 60
    ...
    def start(self):
        ...
        self.video_out = cv2.VideoWriter(self.video_file, self.video_format, float(self.config.rgb_fps),
                                         (self.video_width, self.video_height))
        self.running = True
        super(RGBRecorderThread, self).start()
