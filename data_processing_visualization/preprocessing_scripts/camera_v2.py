...
user = 'P2'
collectionfolder = "Oct14"
dateofcollection = "14-10-2022"

write_dir = f"/Volumes/Vax Storage/processed_data/{user}"
if not os.path.exists(write_dir):
    os.makedirs(write_dir)

video_files = glob.glob(f"/Volumes/Vax Storage/{collectionfolder}/{user}/iphone/*.MP4")
    ...
    for idx, labels_info in enumerate(labels):
        ...
        if (label_start >= video_ts_start) & (label_end < video_ts_end):
            ...
            if not os.path.exists(outfile):
                ...
                response = subprocess.run(['ffmpeg','-ss',label_start_duration,'-to',label_end_duration,'-i',video_file,'-c','copy',outfile], capture_output=True)
