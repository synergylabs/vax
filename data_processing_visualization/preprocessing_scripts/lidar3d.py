"""
This file contains preprocessing logic for lidar3d file
"""
import os.path
...
lidar3d_files = glob.glob(f"/Volumes/Vax Storage/{collectionfolder}/{user}/*/cyglidar3d_*.csv")
...
#     cv2.namedWindow("lidar3d")
#     verify_data = True


def process_chunk(chunk):
    """
    Process one line of data from lidar3d
    :param chunk:
    :return:
    """
...
ts = 0
prev_label=  ""
for lidar3d_file in lidar3d_files:
    remainder = ""
    with open(lidar3d_file, "r") as myFile:
        while True:
            chunk = [remainder]
            chunk_found = False
            ...
            if chunk == "":
                break
            # lidar3d_data.append(process_chunk(chunk))
...
for idx, labels_info in enumerate(labels):
    ...
    pickle.dump(labels_data[idx], open(f"{labels_dir}/lidar3d.pb","wb"))
    json.dump(labels_info, open(f"{labels_dir}/lidar3d_label_info.json","w"))
