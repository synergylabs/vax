"""
Main driver class for  lidar3d recording
...
CYGLIDAR_DEVICE_NAME = 'CYGLIDAR_3D'
...
# SERIAL_BAUDRATE = 250000  # set Baud rate to 9600
...
class Cyglidar3DReaderThread(threading.Thread):
    """
    ...
    def configure_serial(self):
        ...
        data_3d_request = DATA_3D_REQUEST
    ...
    def start(self):
        ...
        # start
        super(Cyglidar3DReaderThread, self).start()
...
class Cyglidar3DWriterThread(threading.Thread):
    """
    ...
    def start(self):
        ...
        # start
        super(Cyglidar3DWriterThread, self).start()
...
class Cyglidar3DDevice(DeviceInterface):
    """
    Device implementation for Cyglidar3DDevice
    """
    ...
    def startReader(self):
        ...
        """
        try:
            self.reader = Cyglidar3DReaderThread(self.sensor_queue,
                                                 self.port_address,
                                                 self.logger,
                                                 self.viz_queue)
            self.reader.start()
    ...
    def startWriter(self):
        ...
        try:
            ...
            if self.write_method == 'csv':
                ...
                self.write_src = f"{self.run_config['experiment_dir']}/cyglidar3d_{time_str}.csv"
                print(f"Write Source: {self.write_src}")
                self.writer = Cyglidar3DWriterThread(self.sensor_queue,
                                                     self.write_src,
                                                     self.logger)
                self.writer.start()
                return True
...
if __name__ == '__main__':
    logger = get_logger("vax_lidar3d")
    default_config_file = 'config/dc_config.json'
    screen_width, screen_height = get_screen_size()
    visualize = True
    window_name = '3D Lidar'
    try:
    ...
    # initialize device
    lidar3dSensor = Cyglidar3DDevice(run_config, sensor_queue, logger, viz_queue)

    # check if available
    if lidar3dSensor.is_available():
        logger.info(f"- Found Sensor {lidar3dSensor.name}-")
        lidar3dSensor.startReader()
        lidar3dSensor.startWriter()

    # run for a given max duration
    try:
        lidar3d_frames = np.zeros((100, 2))
        cv2.namedWindow(window_name)
        cv2.moveWindow(window_name, (screen_width) // 2, (3*screen_height) // 5)
        num_frames = 0
        ...
        while time.time() - start_time < max_duration:
            if visualize:
                if viz_queue.qsize() > 0:
                    ...
                    lidar3d_frame = [viz_queue.get() for _ in range(viz_queue.qsize())][0]
                    img = lidar3d_frame['mat_3d']
                    img = img.astype(np.float32)
                    img = 255 * (img - img.min()) / (1+img.max() - img.min())
                    img_col = cv2.applyColorMap(img.astype(np.uint8), cv2.COLORMAP_DEEPGREEN)
                ...
                if time.time() > start_viz_time + 10.:
                    logger.info(f"Num Lidar3D Frames in last 10 Secs: {num_frames}")
                    num_frames = 0
                    start_viz_time = time.time()
                ...
                if cv2.waitKey(1) == 27:
                    ...
                    break  # esc to quit
        lidar3dSensor.stopWriter()
        lidar3dSensor.stopReader()
        logger.info(f"Data Collection Complete {lidar3dSensor.name}")
    except KeyboardInterrupt:
        lidar3dSensor.stopWriter()
        lidar3dSensor.stopReader()
        cv2.destroyWindow(window_name)
        logger.info(f"Stopped {lidar3dSensor.name}")
