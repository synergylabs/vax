...
def getEnsembleOutput(instance_cv_output, allowed_sensors):
    ...
    all_ts = np.concatenate(
        [instance_cv_output[xr][0] for xr in (instance_cv_output.keys()) if
         ((instance_cv_output[xr][0] is not None))]).astype(int)
    all_gt = np.concatenate(
        [instance_cv_output[xr][2] for xr in (instance_cv_output.keys()) if (instance_cv_output[xr][2] is not None)])
    ...
    total_weight = np.sum(list(SCORE_ENSEMBLE_SENSOR_WEIGHTS.values()))
    for sensor in instance_cv_output.keys():
        ...
        if df_sensor_pred is not None:
            ...
            for col in df_sensor_ts.columns:
                ...
                df_ts_ensemble[col] += (SCORE_ENSEMBLE_SENSOR_WEIGHTS[sensor] / total_weight) * df_sensor_ts[col]
